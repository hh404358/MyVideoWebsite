                        ...
                        <div class="">
                            ...
                            <section>
                                ...
                                <div v-if="haslogin" class="tmsg-replay"
                                    @click="respondMsg(item.id, item.id, item.createUser)">
                                    回复
                                </div>
                            </section>
                        </div>
                            ...
                            <li class="tmsg-c-item" v-for="(citem, cindex) in item.children" :key="'citem' + cindex">
                                <div class="">
                                    ...
                                    <section>
                                        ...
                                        <div v-show="haslogin" class="tmsg-replay"
                                            @click="respondMsg(item.id, citem.id, citem.createUser)">
                                            回复
                                        </div>
                                    </section>
                                </div>
...
<script>
import { sendComment, getComment } from '../api/comment.js'
    ...
    methods: { //事件处理器
        setData(initData, result) {
            ...
            if (initData) {
                ...
                this.commentList = msg; console.log("commentlist:"+this.commentList);
            } else {
                //加载更多
        ...
        sendMsg: function () {//留言
            ...
            if (that.textarea) {
                that.sendTip = '咻~~';
                sendComment(that.aid, that.rootId, that.toCommentId, that.toCommentUserId, that.textarea).then((response) => {
                    that.textarea = '';
                    that.rootId = -1;
                    that.toCommentId = -1;
        ...
        showCommentList: function (initData) {//评论列表
            ...
            that.queryParams.videoId = that.aid
            //判断当前用户是否登录
            var token = getToken();
            if (token) {
                that.haslogin = true;
            ...
            //公用设置数据方法
            getComment(that.queryParams).then((response) => {
...
.tmsg-c-item video header {
    margin-bottom: 10px;
}

.tmsg-c-item video header img {
    width: 65px;
    height: 65px;
    border-radius: 50%;
    float: left;
...
.tmsg-c-item video header img:hover {
    transform: rotate(360deg);
    -webkit-transform: rotate(360deg);
}

.tmsg-c-item video header .i-name {
    font-size: 14px;
...
.tmsg-c-item video header .i-class {
    display: inline-block;
    margin-left: 10px;
    background: #dff0d8;
    color: #3c763d;
...
.tmsg-c-item video header .i-time {
    color: #aaa;
    font-size: 12px;
}

.tmsg-c-item video section {
    margin-left: 80px;
}

.tmsg-c-item video section p img {
    vertical-align: middle;
}

.tmsg-c-item video section .tmsg-replay {
    margin: 10px 0;
    font-size: 12px;
    color: #64609E;
    cursor: pointer;
