    ...
    export default {
        ...
        methods: { //事件处理器
            ...
            newRegister:function(){//注册提交
                ...
                if(reg1.test(that.nphonenunmber)){
                ...
                if(!that.nusernameErr&&!that.nemailErr&&!that.npasswordErr){
                    ...
                    userRegister(that.nusername,that.nnickName,that.nemail,that.npassword,that.nphonenunmber).then((response)=>{
                         //注册成功后调整到登录
                         that.goLogin()
                    }).catch((error)=>{
